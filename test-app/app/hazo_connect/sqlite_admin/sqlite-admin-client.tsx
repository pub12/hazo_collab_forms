...
export default function SqliteAdminClient({
  ...
  async function loadSchemaAndData(tableName: string) {
    ...
    try {
      const schemaResponse = await fetch(`/api/hazo_connect/sqlite/schema?table=${encodeURIComponent(tableName)}`)
  ...
  async function loadData(
    ...
    try {
      ...
      const response = await fetch(`/api/hazo_connect/sqlite/data?${params.toString()}`)
      if (!response.ok) {
  ...
  async function refreshTables() {
    ...
    try {
      const response = await fetch("/api/hazo_connect/sqlite/tables")
      if (!response.ok) {
        throw new Error(await response.text())
      }
  ...
  async function handleInsertRow(data: Record<string, unknown>) {
    ...
    try {
      const response = await fetch("/api/hazo_connect/sqlite/data", {
        method: "POST",
        headers: {
