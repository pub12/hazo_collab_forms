/**
 * Inputbox Component test page
 * Documents the HazoCollabFormInputbox component
 */
...
export default function InputboxComponentPage() {
  const config = COMPONENT_PAGES.find(c => c.path === '/components/inputbox');
  ...
  const base_props: PropsTableData[] = [
    {
      ...
      is_inherited: true,
      default_value: 'Sample Input Field',
    },
    {
      name: 'error',
  ...
  // Inputbox-specific props
  const inputbox_props: PropsTableData[] = [
    {
      name: 'value',
      type: 'string',
      required: true,
    ...
    {
      name: 'input_wrapper_class_name',
      type: 'string | undefined',
      required: false,
      category: 'Controllable Element',
      description: 'Custom className for the input wrapper',
    },
    {
      name: 'type',
      type: 'string | undefined',
      required: false,
      category: 'Controllable Element',
      description: 'Input type (text, email, password, etc.). Extends standard HTML input attributes',
    },
    {
      name: 'placeholder',
      type: 'string | undefined',
    ...
    {
      name: 'input_type',
      type: '"mixed" | "numeric" | "email" | "alpha" | undefined',
      required: false,
      category: 'Input Field',
      description: 'Input type validation. "mixed" allows any characters, "numeric" allows numbers only, "email" validates email format, "alpha" allows letters only',
    },
    {
      name: 'text_len_min',
      type: 'number | undefined',
      required: false,
      category: 'Input Field',
      description: 'Minimum text length allowed',
    },
    {
      name: 'text_len_max',
      type: 'number | undefined',
    ...
    },
    {
      ...
      required: false,
      category: 'Input Field',
      description: 'Minimum numeric value allowed (used when input_type is "numeric")',
    },
    {
      name: 'num_max',
      type: 'number | undefined',
      required: false,
      category: 'Input Field',
      description: 'Maximum numeric value allowed (used when input_type is "numeric")',
    },
    {
      name: 'regex',
    ...
    },
    {
      ...
      required: false,
      category: 'Input Field',
      description: 'Number of decimal places allowed (used when input_type is "numeric")',
    },
    {
      name: 'format_guide',
      type: 'string | undefined',
      required: false,
      category: 'Input Field',
      description: 'Format guide text to display to help users understand the expected input format',
    },
    {
      name: 'format_guide_info',
      type: 'boolean | undefined',
      required: false,
      category: 'Controllable Element',
      description: 'Whether to show format guide info. When true, displays the format_guide text',
    },
  ];

  const props_table_data = [...base_props, ...inputbox_props];
  ...
  const example_component = (prop_values: Record<string, any>) => {
    ...
    const component_props: Record<string, any> = {
      ...
      label: prop_values.label || 'Sample Input Field',
    };
    ...
    if (prop_values.type !== undefined && prop_values.type !== '') {
      component_props.type = prop_values.type;
    }
    ...
    // HazoUiFlexInput validation props
    if (prop_values.input_type !== undefined && prop_values.input_type !== '') {
      component_props.input_type = prop_values.input_type;
    }
    if (prop_values.text_len_min !== undefined && prop_values.text_len_min !== '') {
      const text_len_min_value = parseInt(String(prop_values.text_len_min));
      if (!isNaN(text_len_min_value)) {
        component_props.text_len_min = text_len_min_value;
      }
    }
    if (prop_values.text_len_max !== undefined && prop_values.text_len_max !== '') {
      const text_len_max_value = parseInt(String(prop_values.text_len_max));
      if (!isNaN(text_len_max_value)) {
        component_props.text_len_max = text_len_max_value;
      }
    }
    if (prop_values.num_min !== undefined && prop_values.num_min !== '') {
      const num_min_value = parseFloat(String(prop_values.num_min));
      if (!isNaN(num_min_value)) {
        component_props.num_min = num_min_value;
      }
    }
    if (prop_values.num_max !== undefined && prop_values.num_max !== '') {
      const num_max_value = parseFloat(String(prop_values.num_max));
      if (!isNaN(num_max_value)) {
        component_props.num_max = num_max_value;
      }
    }
    if (prop_values.regex !== undefined && prop_values.regex !== '') {
      component_props.regex = prop_values.regex;
    }
    if (prop_values.num_decimals !== undefined && prop_values.num_decimals !== '') {
      const num_decimals_value = parseInt(String(prop_values.num_decimals));
      if (!isNaN(num_decimals_value)) {
        component_props.num_decimals = num_decimals_value;
      }
    }
    if (prop_values.format_guide !== undefined && prop_values.format_guide !== '') {
      component_props.format_guide = prop_values.format_guide;
    }
    if (prop_values.format_guide_info !== undefined) {
      component_props.format_guide_info = prop_values.format_guide_info === true || prop_values.format_guide_info === 'true';
    }
    // HazoChat props
    ...
    // Test callback for file upload
    const handle_file_upload = async (file_data: any, ref: any) => {
      console.log('File uploaded:', file_data);
      // Show spinner for 2 seconds
      await new Promise(resolve => setTimeout(resolve, 2000));
      set_example_value(file_data.file_name);
    };
    
    component_props.file_processor = handle_file_upload;

    return (
      <div className="cls_inputbox_example_container space-y-4">
        <HazoCollabFormInputbox
          ...
          file_processor={component_props.file_processor}
          input_type={component_props.input_type}
          text_len_min={component_props.text_len_min}
          text_len_max={component_props.text_len_max}
          num_min={component_props.num_min}
          num_max={component_props.num_max}
          regex={component_props.regex}
          ...
          onChange={(val) => {
            ...
            if (val.length > 0 && val.length < 3) {
              set_example_error('Input must be at least 3 characters');
            } else {
        ...
        <div className="text-sm text-muted-foreground">
          <p>Current value: {example_value || '(empty)'}</p>
