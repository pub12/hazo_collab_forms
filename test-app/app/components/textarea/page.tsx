 ...
 * Documents the HazoCollabFormTextArea component
 */
...
import { HazoCollabFormTextArea } from 'hazo_collab_forms';

/**
 * TextArea Component test page
 */
export default function TextAreaComponentPage() {
  const config = COMPONENT_PAGES.find(c => c.path === '/components/textarea');
  ...
  // Default test recipient user ID for chat functionality
  const default_recipient_user_id = '2775a808-88d9-4e43-aae9-47420ae003dc';

  // Base props (inherited) - same as inputbox
  const base_props: PropsTableData[] = [
    { name: 'label', type: 'string', required: true, category: 'Input Field', description: 'Label text for the field', is_inherited: true, default_value: 'Sample TextArea Field' },
  ...
  const textarea_props: PropsTableData[] = [
    {
      name: 'value',
      type: 'string',
      required: true,
    ...
    {
      name: 'textarea_wrapper_class_name',
      type: 'string | undefined',
      required: false,
      category: 'Controllable Element',
      description: 'Custom className for the textarea wrapper',
    },
    {
      name: 'rows',
    ...
    },
    {
      ...
      required: false,
      category: 'Controllable Element',
      description: 'Minimum length of the textarea',
    },
    {
      name: 'autoFocus',
      type: 'boolean | undefined',
  ...
  const props_table_data = [...base_props, ...textarea_props];
  ...
  const example_component = (prop_values: Record<string, any>) => {
    ...
    const component_props: Record<string, any> = {
      ...
      label: prop_values.label || 'Sample TextArea Field',
    };
    ...
    if (prop_values.rows !== undefined && prop_values.rows !== '') {
      const rows_value = parseInt(String(prop_values.rows));
      if (!isNaN(rows_value)) {
        component_props.rows = rows_value;
      }
    }
    ...
    return (
      <div className="cls_textarea_example_container space-y-4">
        <HazoCollabFormTextArea
          ...
          onChange={(val) => {
            ...
            if (val.length > 0 && val.length < 10) {
              set_example_error('Text must be at least 10 characters');
            } else {
        ...
        <div className="text-sm text-muted-foreground">
          <p>Character count: {example_value.length}</p>
